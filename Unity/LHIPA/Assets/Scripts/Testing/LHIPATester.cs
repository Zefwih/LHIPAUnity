...
public class LHIPATester : MonoBehaviour
{
    ...
    public string inputFilePath = "Assets/pupil_diameter_signals.json";
    [Tooltip("Path to output logfile with LHIPA values.")]
    public string outputFilePath = "Assets/lhipa_results";
    ...
    // Generate deterministic eye diameter test arrays
    private List<float[]> GenerateTestArrays()
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            for (int j = 0; j < numPoints; j++)
            {
                ...
                array[j] = Mathf.Round((baseline + amplitudeVariation * baseFrequency + noise) * 100) / 100; // Round to 2 decimals
            }
    ...
    public string TestLHIPAAndLog(bool useInputFile)
    {
        ...
        using (StreamWriter writer = new StreamWriter(newPathFile, true))
        {
            ...
            for (int i = 0; i < testArrays.Count; i++)
            {
                ...
                float lhipaResult = Mathf.Round(LHIPA.CalculateLHIPA(testArray, durationInSeconds, modMaxCorrectionThreshold, consoleLog) * 1000000) / 1000000; // Round to 6 decimals
