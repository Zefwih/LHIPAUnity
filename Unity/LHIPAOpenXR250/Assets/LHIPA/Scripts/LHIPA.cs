...
namespace lhipa
{
    ...
    /// A Psychophysiological Measure of Cognitive Workload".
            ...
            float modMaxCorrectionThreshold = 0f, bool debugLog = false)
        {
            if (pupilData == null || pupilData.Length == 0 || durationInSeconds <= 0)
                throw new ArgumentException(
                    "Incorrect input. Please check if you submitted a correct pupilData array and a non negative durationInSeconds.");
            ...
            if (debugLog)
                Debug.Log(
                    "1. Detail coefficients average high - low: " + highFreq.Average() + " - " + lowFreq.Average());
            ...
            if (debugLog)
                Debug.Log("2. Normalized coefficients data average high - low: " + normalizedHigh.Average() + " - " +
                          normalizedLow.Average());
            ...
            int ratioLength = Math.Min(normalizedLow.Length,
                normalizedHigh.Length * (int)Mathf.Pow(2, lfLevel - hfLevel));
            ...
            for (int i = 0; i < ratioLength; i++)
            {
                ...
                {
                    ratio[i] = normalizedLow[i] /
                               (normalizedHigh[highIndex] + 1e-6f); // +1e-6 to prevent division with 0
                }
                else
                {
            ...
            if (debugLog)
                Debug.Log("4. modulus maxima length: " + modulusMaxima.Length + " - modulus maxima average: " +
                          modulusMaxima.Average());
            if (debugLog) Debug.Log($"[{string.Join(" | ", modulusMaxima)}]");

            // 5. Calculation of Threshold (universal Threshold)
            float lambdaUniv = Mathf.Sqrt(2.0f * Mathf.Log(modulusMaxima.Length) / Mathf.Log(2.0f)) *
                               StandardDeviation(modulusMaxima);
            ...
            if (debugLog)
                Debug.Log("6. Universal thresholded length: " + thresholded.Length +
                          " - Universal thresholded average: " + thresholded.Average());
            ...
            if (debugLog)
                Debug.Log("7. maximaCount: " + maximaCount + ", Duration: " + durationInSeconds + ", Sampling Rate: " +
                          samplingRate);
            ...
            lhipa = multiplicativeCorrectionParameter != 0
                ? lhipa / multiplicativeCorrectionParameter -
                  (lhipa > additiveCorrectionParameter ? additiveCorrectionParameter : 0)
