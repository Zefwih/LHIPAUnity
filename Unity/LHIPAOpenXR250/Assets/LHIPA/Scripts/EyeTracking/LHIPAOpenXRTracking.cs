...
{
    [Header("Eye Diameter Logging Parameters")]
    [Tooltip("Path to output logfile with LHIPA values.")]
    public string outputFilePath = "Assets/pupil_data";
    ...
    private int maxSamples = 10000000; // Maximal number of von dates in pupilDiameter array (storage space limitation)
    ...
    public float[] StopEyeTrackingRecordingAndCalculateLHIPA()
    {
        ...
        float resultLHIPAvalue = LHIPA.CalculateLHIPA(dataSlice, duration, modMaxCorrectionThreshold, showDetailedCalculationLog);
        Debug.Log("Result LHIPA value: " + resultLHIPAvalue + ", Duration of recording: " + duration + ", Sampling rate: " + samplingRate);
        return new [] { resultLHIPAvalue, duration, samplingRate}; // float: 0. value: LHIPA, 1. value: duration, 2. value: sampling rate
    }
        ...
        if (isCalculating || isRecording)
        {
            ...
            if (isCalculating)
            {
                ...
                if (Time.time - startTimeLive >= calculationInterval)
                {
                    ...
                    currentLiveLHIPAValue = LHIPA.CalculateLHIPA(dataSlice, duration, modMaxCorrectionThreshold, showDetailedCalculationLog);
